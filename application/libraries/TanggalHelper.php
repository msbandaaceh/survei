...
class TanggalHelper{
	...
	public function convertDate($str){
		if(preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $str)){
			$date = (date_format(date_create($str),'Y/M/d'));
			$dates = explode("/", $date);
			return $this->monthName($dates[1])." ".$dates[0];
		}else{
			return " - ";
		}
	}
    ...
    public function convertMonthDate($str) {
        if(preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $str)){
			$date = (date_format(date_create($str),'Y/M/d'));
			$dates = explode("/", $date);
			return $this->monthName($dates[1])." ".$dates[0];
	...
	public function konversiMasaKerja($number) {
		$tahun = floor($number / 365);
		$bulan = floor(floor(($number - ($tahun * 365))/30.5));
		return $tahun." tahun ".$bulan." bulan";
	}

	public function konversiMasaKerjaTahun($number) {
		$tahun = floor($number / 365);
		return $tahun;
	}

	public function konversiMasaKerjaBulan($number) {
		$tahun = floor($number / 365);
		$bulan = floor(floor(($number - ($tahun * 365))/30.5));
		return $bulan;
	}
    ...
    public function konversiJam($timeString)
    {
        // Mendapatkan jam dan menit dari string waktu
        $formattedTime = substr($timeString, 0, 5); // Mengambil 5 karakter pertama (HH:mm)
